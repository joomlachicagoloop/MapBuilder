	...
	<table>
		<tr>
			<td width="100%">
				...
				<input type="text" name="filter_search" id="filter-search_" value="<?php echo $this->filter->filter_search; ?>" />
	...
	<table class="adminlist">
		<thead>
			<tr>
				...
				<th class="title" width="20%">
					<?php echo JHTML::_('grid.sort', JText::_('Marker Name'), 'marker_name', $this->filter->filter_order_Dir, $this->filter->filter_order, 'filter'); ?>
				</th>
				<th width="20%">
					<?php echo JHTML::_('grid.sort', JText::_('Map Name'), 'map_name', $this->filter->filter_order_Dir, $this->filter->filter_order, 'filter'); ?>
				</th>
				<th width="5%" nowrap="nowrap">
					<?php echo JHTML::_('grid.sort', JText::_('Published'), 'm.published', $this->filter->filter_order_Dir, $this->filter->filter_order, 'filter'); ?>
				</th>
				<th width="10%" nowrap="nowrap">
					<?php echo JHTML::_('grid.sort', JText::_('Order'), 'ordering', $this->filter->filter_order_Dir, $this->filter->filter_order, 'filter');?>
					<?php echo JHTML::_('grid.order', $this->items); ?>
				</th>
				<th nowrap="nowrap">
					<?php echo JHTML::_('grid.sort', JText::_('Access'), 'm.access', $this->filter->filter_order_Dir, $this->filter->filter_order, 'filter'); ?>
				</th>
				<th>
		...
		for($i=0; $i < count($this->items); $i++){
			...
			<tr class="row<?php echo $k; ?>">
				...
				<td>
					<?php echo ($row->map_name) ? $row->map_name : "none"; ?>
				</td>
				<td align="center">
					<?php echo JHtml::_('jgrid.published', $row->published, $i, 'markers.', true, 'cb'); ?>
				</td>
				<td class="order">
					<span><?php echo $this->page->orderUpIcon( $i, ($i > 0), 'markers.orderup', 'Move Up'); ?></span>
					<span><?php echo $this->page->orderDownIcon( $i, count($this->items), true, 'markers.orderdown', 'Move Down'); ?></span>
					<input type="text" name="order[]" size="5" value="<?php echo $row->ordervalue; ?>" class="text_area" style="text-align: center" />
				</td>
				<td>
					<?php echo $row->access; ?>
				</td>
				<td>
					<?php echo JText::_( strip_tags($row->marker_description) ); ?>
				</td>
				<td>
