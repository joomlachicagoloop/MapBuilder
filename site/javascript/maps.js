var Maps = Class.create({
	initialize: function(){
		...
		this.retrieve = this.findMarker.bind(this);
		this.show = this.showInfoWindow.bind(this);
		this.latX = /lat(-?\d+\.\d+)/;
		this.lngX = /lng(-?\d+\.\d+)/;
	...
	loadData: function(someId){
		new Ajax.Request("/index.php?option=com_maps&layout=ajax&id=" + someId, { method: 'GET', onSuccess: this.parse, onFailure: this.error });
	},
	
	parseData: function(someAjax){
		var someData = XMLObjectifier.xmlToJSON(someAjax.responseXML);
		var someId = someData.record[0].maps_id[0].Text;
		var someWindow = this.infoWindow;
		someData.record.each(function(someRecord){
			var someMap = this.find(someId);
			var markerData = {
				lat: someRecord.marker_lat[0].Text,
				lng: someRecord.marker_lng[0].Text,
				alias: someRecord.marker_alias[0].Text,
				title: someRecord.marker_name[0].Text,
			...
			google.maps.event.addListener(someMarker, "click", function(){
				var content = "<h3>" + someRecord.marker_name[0].Text + "</h3>";
				content += "<p>" + someRecord.marker_description[0].Text + "</p>";
				someWindow.setContent(content);
				someWindow.setOptions({ maxWidth: 300 });
				someWindow.open(someMarker.getMap(), someMarker);
			});
			markerData.marker = someMarker;
	...
	findMap: function(someId){
		return this.maps.find(function(someMap){
	...
	findMarker: function(someTitle){
		return this.markers.find(function(someMarker){
			return someMarker.alias == someTitle;
		}, this);
	},
	
	showInfoWindow: function(someEvent){
		var someAlias = someEvent.target.title
		if(!someAlias){
			someAlias = someEvent.target.up('div').title;
		}
		var markerData = this.retrieve(someAlias);
		if(markerData){
			var content = "<h3>" + markerData.title + "</h3>";
			content += "<p>" + markerData.description + "</p>";
			this.infoWindow.setContent(content);
			this.infoWindow.setOptions({ maxWidth: 300 });
			this.infoWindow.open(markerData.marker.map, markerData.marker);
		}
	}
});

var initMaps = function(){ myMaps = new Maps(); }
Element.observe(window, "load", initMaps);
